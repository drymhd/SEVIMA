  ...
  <div class="card-body">
   <form autocomplete="off" @submit.prevent="tambah">
      ...
      <select id="inputState" class="form-control" v-model="form.kelas_id">
        <option v-for="data in datakelas" :value="data.id">{{data.nama_kelas}}</option>
      </select>
    </div>

                <div class="form-group">
                    <label for="name">NISN</label>
                    <input type="text" id="name" class="form-control" placeholder="NISN" oninput="this.value = this.value.replace(/\D/g, '')" required v-model="form.nisn">
                </div>
                <div class="form-group">
    ...
    export default {
       ...
       methods: {
         tambah(){
          var app = this;
          if(app.type != 'tambah'){
            app.$http.post('admin/siswa/edit', {data: app.form}).then((res) => {
          ...
          app.$http.post('admin/siswa/tambah', {data: app.form}).then((res) => {
        ...
        edit(id){
          ...
          app.show = true;
          app.type = 'edit';
          app.$http.get('admin/siswa/get/'+id).then((res) => {
        ...
        hapus(id){
          var app = this;
          app.$http.get("admin/siswa/hapus/"+id).then((res) => {
